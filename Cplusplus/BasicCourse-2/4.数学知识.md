...
1 ~ N 中与 N 互质的数的个数被称为欧拉函数，记为 $\phi(N)$，如果一个数N可以质因数分解为 $p_1^{\alpha_1}*p_2^{\alpha_2}*...*p_k^{\alpha_k}$ ，则  
$$
...
1~n中和n互质的数有 $\phi(n)$ 个，假设分别是：$b_1,b_2,...,b_{\phi(n)}$，因为a与n互质，则有：$a*b_1,a*b_2,...,a*b_{\phi(n)}$ 均与n互质，并且两两互不相同(反证法)，所以这两组数是同一组数(在模n的前提下)，只不过是位置调换了一下，因此这两组数的乘积同余，所以有：
$$
a^{\phi(n)}*(b_1b_2...b_{\phi(n)}) \equiv (b_1b_2...b_{\phi(n)}) (mod \ n)
$$
根据[同余的性质](https://zhidao.baidu.com/question/93176537.html)，有：
$$
a^{\phi(n)} \equiv 1 (mod \ n)
$$
证毕！
